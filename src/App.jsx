...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-gray-900 text-white font-sans overflow-hidden">
      ...
      <style>{`
        @keyframes float {0%,100%{transform:translate(-10%,-10%) scale(1);}50%{transform:translate(10%,10%) scale(1.05);}}
        @keyframes float2 {0%,100%{transform:translateY(0) translateX(0) scale(1);}50%{transform:translateY(-20px) translateX(10px) scale(1.05);}}
        @keyframes float3 {0%,100%{transform:translateY(0) translateX(0) scale(1);}50%{transform:translateY(15px) translateX(-15px) scale(1.05);}}
      `}</style>
      ...
      <section className="flex flex-col items-center justify-center text-center min-h-screen px-6">
        ...
        <div className="mt-6 flex gap-4">
          <a href="/resume.pdf" download className="bg-teal-500 px-6 py-3 rounded-xl font-semibold shadow-md hover:bg-teal-600">
            Download Resume
          </a>
      ...
      <section id="projects" className="py-20">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            {projects.map(p => (
              <div key={p.id} className="bg-white/10 rounded-3xl overflow-hidden shadow-lg transform hover:scale-105 hover:shadow-[0_0_20px_#06b6d4] transition-all duration-300">
                <img src={p.img} alt={p.title} className="w-full h-56 object-cover rounded-t-3xl"/>
                <div className="p-6">
                  <h4 className="text-xl font-semibold mb-2">{p.title}</h4>
                  <p className="text-gray-300 mb-4">Modern UI/UX project with fluid colors and smooth interactions.</p>
                  <a href={p.link} target="_blank" rel="noopener noreferrer" className="inline-block bg-teal-500 px-4 py-2 rounded-lg hover:bg-teal-600">View Project</a>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="about" className="py-20 bg-black/30">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-gray-300 text-lg">
            I am a creative UI/UX designer with experience in designing modern mobile and web applications.
            I focus on delivering intuitive interfaces and engaging user experiences with fluid animations and clean layouts.
          </p>
        </div>
      </section>
      ...
      <section id="contact" className="py-20">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-gray-300 mb-6">Feel free to reach out for collaborations or project inquiries.</p>
          <a href="mailto:vinod@example.com" className="bg-teal-500 px-6 py-3 rounded-xl font-semibold hover:bg-teal-600">Email Me</a>
        </div>
      </section>
    </div>
  );
